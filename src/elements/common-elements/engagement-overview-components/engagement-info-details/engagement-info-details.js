...
Polymer({
  ...
  _setSharedIpWith: function(basePermissionPath) {
    const sharedIpWithOptions = this.getChoices(`${basePermissionPath}.shared_ip_with.child`);
    return sharedIpWithOptions || [];
  },

  validate: function() {
    const orderField = this.$.purchaseOrder;
    const orderValid = orderField && orderField.validate(orderField);

    const elements = Polymer.dom(this.root).querySelectorAll('.validate-field');
    let valid = true;
    ...
    const periodStart = Polymer.dom(this.root).querySelector('#periodStartDateInput');
    const periodEnd = Polymer.dom(this.root).querySelector('#periodEndDateInput');
  ...
  getEngagementData: function() {
    const data = {};
    const agreementId = _.get(this, 'data.agreement.id');
    const originalAgreementId = _.get(this, 'originalData.agreement.id');
  ...
  _setEngagementTypes: function(basePermissionPath) {
    const types = this.getChoices(`${basePermissionPath}.engagement_type`);
    if (!types) {return;}
  ...
  _hideField: function(fieldName, basePermissionPath) {
    ...
    const collectionNotExists = !this.collectionExists(path, 'POST') &&
