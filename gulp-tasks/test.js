...
module.exports = function testElements(done) {
    ...
    function runTests() {
        ...
        let tests;
        if (argv.noxvfb) {
            tests = spawn('node', ['./node_modules/.bin/wct', `./build/tests/${indexFileName}`], { stdio: 'inherit' });
        } else {
            tests = spawn('xvfb-run', ['-a', './node_modules/.bin/wct', `./build/tests/${indexFileName}`], { stdio: 'inherit' });
        }
